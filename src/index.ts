...
async function promptForSupabaseConfig() {
  ...
  console.log(
    chalk.gray('You can find these values in your Supabase project settings.\n')
  )
    ...
    .action(async (name, options) => {
      try {
        ...
        if (options.install === false)
          projectOptions.installDependencies = false
        ...
        if (!projectOptions.installDependencies) {
          console.log(
            chalk.white(`   ${projectOptions.packageManager} install`)
          )
        }

        if (!projectOptions.setupSupabase) {
          console.log(
            chalk.white('   Set up your Supabase credentials in .env.local')
          )
        }
